...
public:
    ...
    Product getProducts(int index) const { return products[index]; }
...
int main()
{
    ...
    while (true)
    {
        ...
        if (choice == "1")
        {
            ...
            if (productName == "back")
                continue;
            int productNum = std::stoi(productName);
            ...
            if (target == "wishlist")
            {
                store.addToWishlist(store.getProducts(productNum - 1).getName());
            }
            else if (target == "cart")
            {
                store.addToCart(store.getProducts(productNum - 1).getName());
            }
            else
            {
